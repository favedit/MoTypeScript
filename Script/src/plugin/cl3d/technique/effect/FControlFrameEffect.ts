...
export class FControlFrameEffect
   ...
   //==========================================================
   public drawRenderable(pg, pr) {
      var c = this.graphicContext;
      var p = this.program;
      // 获得参数
      var vcp = pg.calculate(ERegionParameter.CameraPosition);
      var vld = pg.calculate(ERegionParameter.LightDirection);
      // 绑定材质
      var m = pr.material();
      var mi = m.info();
      this.bindMaterial(m);
      ...
      p.setParameter('vc_vp_matrix', pg.calculate(ERegionParameter.CameraViewProjectionMatrix));
      ...
      // 绑定所有属性流
      this.bindAttributes(pr);
      // 绑定所有取样器
      this.bindSamplers(pr);
      // 绘制处理
