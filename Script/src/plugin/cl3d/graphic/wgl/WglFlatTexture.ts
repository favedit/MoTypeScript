...
export class WglFlatTexture extends FlatTexture {
   ...
   public update() {
      ...
      var code = WglUtil.convertSamplerFilter(graphic, this.filterMagCd);
      if (code) {
         graphic.texParameteri(graphic.TEXTURE_2D, graphic.TEXTURE_MAG_FILTER, code);
      }
      var code = WglUtil.convertSamplerFilter(graphic, this.wrapS);
      if (code) {
         graphic.texParameteri(graphic.TEXTURE_2D, graphic.TEXTURE_WRAP_S, code);
      }
      var code = WglUtil.convertSamplerFilter(graphic, this.wrapT);
      if (code) {
