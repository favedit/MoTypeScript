...
import {WglVertexBuffer} from './WglVertexBuffer';
import {WglIndexBuffer} from './WglIndexBuffer';
import {WglFlatTexture} from './WglFlatTexture';
import {WglCubeTexture} from './WglCubeTexture';
import {WglRenderTarget} from './WglRenderTarget';
import {WglProgram} from './WglProgram';
import {WglLayout} from './WglLayout';
...
// @history 141230
//==========================================================
export class WglContext extends GraphicContext {
   // @attribute
   public setting: WglSetting;
   public handle: any;
   ...
   public constructor() {
      super();
      this.capability = new ContextCapability();
   ...
   public setup(hCanvas: HTMLCanvasElement, handle: any) {
      ...
      this.handle = handle;
      // 设置状态
      this.setDepthMode(true, DepthModeEnum.LessEqual);
      this.setCullingMode(true, CullModeEnum.Front);
      // 获得渲染信息
   ...
   public parameters() {
      ...
      var names = [
         ...
         'LINE_WIDTH',
         'MAX_COLOR_ATTACHMENTS', 'MAX_COMBINED_TEXTURE_IMAGE_UNITS', 'MAX_CUBE_MAP_TEXTURE_SIZE', 'MAX_DRAW_BUFFERS',
   ...
   // @return 顶点缓冲
   //==========================================================
   public createProgram(clazz: Function = WglProgram) {
      var program = this.createObject(WglProgram);
   ...
   // @return 顶点缓冲
   //==========================================================
   public createLayout(clazz: Function = WglLayout) {
      var layout = ClassUtil.create(WglLayout);
   ...
   // @return 顶点缓冲
   //==========================================================
   public createVertexBuffer(clazz: Function = WglVertexBuffer) {
   ...
   // @return 索引缓冲
   //==========================================================
   public createIndexBuffer(clazz: Function = WglIndexBuffer) {
      var buffer = this.createObject(clazz);
      this._storeBuffers.push(buffer);
   ...
   // @return 平面渲染纹理
   //==========================================================
   public createFlatTexture(clazz: Function = WglFlatTexture) {
   ...
   // @return 立方渲染纹理
   //==========================================================
   public createCubeTexture(clazz: Function = WglCubeTexture) {
      var texture = this.createObject(clazz);
      this._storeTextures.push(texture);
   ...
   // @return 渲染目标
   //==========================================================
   public createRenderTarget(clazz: Function = WglRenderTarget) {
      var texture = this.createObject(clazz);
   ...
   public setDepthMode(depthFlag, depthCd) {
      ...
      if (depthFlag && (this._depthModeCd != depthCd)) {
         var depthCode = WglUtil.convertDepthMode(graphic, depthCd);
   ...
   public setCullingMode(cullFlag, cullCd) {
      ...
      if (cullFlag && (this._cullModeCd != cullCd)) {
         var cullValue = WglUtil.convertCullMode(graphic, cullCd);
   ...
   public setBlendFactors(blendFlag, sourceCd, tagetCd) {
      ...
      if (blendFlag && ((this._blendSourceCd != sourceCd) || (this._blendTargetCd != tagetCd))) {
         var sourceValue = WglUtil.convertBlendFactors(graphic, sourceCd);
         var tagetValue = WglUtil.convertBlendFactors(graphic, tagetCd);
   ...
   public bindConst(shaderCd, slot, formatCd, data, count) {
      ...
      // 修改数据
      switch (formatCd) {
         case ParameterFormatEnum.Float1: {
            // 修改数据
         ...
         case ParameterFormatEnum.Float2: {
            // 修改数据
         ...
         case ParameterFormatEnum.Float3: {
            // 修改数据
         ...
         case ParameterFormatEnum.Float4: {
            // 修改数据
         ...
         case ParameterFormatEnum.Float3x3: {
            // 修改数据
         ...
         case ParameterFormatEnum.Float4x4: {
            // 修改数据
            let bytes = null;
   ...
   //==========================================================
   public bindVertexBuffer(slot, vertexBuffer: VertexBuffer, offset: number, formatCd: any) {
      var graphic = this.handle;
      var result = true;
   ...
   //==========================================================
   public drawTriangles(indexBuffer: IndexBuffer, offset, count) {
      var graphic = this.handle;
      var result = true;
      // 设置参数
      if (offset == null) {
      ...
      var strideValue = WglUtil.convertIndexStride(graphic, strideCd);
      var offsetValue = 0;
      switch (strideCd) {
      ...
      var drawModeValue = WglUtil.convertDrawMode(graphic, drawModeCd);
      switch (drawModeCd) {
