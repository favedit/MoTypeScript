...
import {EffectConsole} from './EffectConsole';
...
// @history 141230
//==========================================================
export class Effect extends Content {
   // @attribute
   public ready: boolean;
   public code: string;
   public flag: string;
   // @attribute
   public stateFillCd = FillModeEnum.Face;
   public stateCullCd = CullModeEnum.Front;
   public stateDepth = true;
   public stateDepthCd = DepthModeEnum.LessEqual;
   public stateDepthWrite = true;
   public stateBlend = true;
   public stateBlendSourceCd = BlendModeEnum.SrcAlpha;
   public stateBlendTargetCd = BlendModeEnum.OneMinusSrcAlpha;
   ...
   // @param count 总数
   //==========================================================
   public drawRenderables(region: Region, renderables: Objects<Renderable>, offset, count) {
      // 选择技术
   ...
   // @param count:Integer 总数
   //==========================================================
   public drawRegion(region: Region, offset: number, count: number) {
      // 根据效果类型进行分组
   ...
   public loadConfig(xconfig) {
      ...
      for (var i = 0; i < count; i++) {
         ...
         if (xnode.isName('State')) {
            ...
            if (name == 'fill_mode') {
               this.stateFillCd = EnumUtil.encode(FillModeEnum, value);
            } else if (name == 'cull_mode') {
               this.stateCullCd = EnumUtil.encode(CullModeEnum, value);
            } else if (name == 'depth_mode') {
               this.stateDepth = true;
               this.stateDepthCd = EnumUtil.encode(DepthModeEnum, value);
            } else if (name == 'depth_write') {
               this.stateDepthWrite = BooleanUtil.parse(value);
            } else if (name == 'blend_mode') {
               this.stateBlend = BooleanUtil.parse(value);
               if (this.stateBlend) {
                  this.stateBlendSourceCd = EnumUtil.encode(BlendModeEnum, xnode.get('source'));
                  this.stateBlendTargetCd = EnumUtil.encode(BlendModeEnum, xnode.get('target'));
               }
            ...
            // 设置参数
            var parameter: ProgramParameter = ClassUtil.create(ProgramParameter);
            ...
            // 设置取样
            var sampler: ProgramSampler = ClassUtil.create(ProgramSampler);
      ...
      // 建立代码模板
      var vertexTemplate = this.vertexTemplate = ClassUtil.create(ShaderTemplate);
      vertexTemplate.load(this._vertexSource);
      var fragmentTemplate = this.fragmentTemplate = ClassUtil.create(ShaderTemplate);
   ...
   //==========================================================
   public build(effectInfo: EffectInfo) {
      ...
      program.upload(ShaderEnum.Vertex, formatSource);
      // 生成像素代码
      ...
      var formatSource = StringUtil.formatLines(source);
      program.upload(ShaderEnum.Fragment, formatSource);
      // 编译处理
      program.build();
      program.link();
   }
   ...
   //==========================================================
   public load() {
      var xconfig = ServiceUtil.find(EffectConsole).loadConfig(this.code);
