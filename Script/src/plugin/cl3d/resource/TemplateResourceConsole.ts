...
// @history 150108
//==========================================================
export class TemplateResourceConsole extends Service {
   // 模板集合
   public templates:Dictionary<TemplateResource> = null;
   // 资源控制台
   ...
   public constructor() {
      super();
      // 设置属性
      this.templates = new Dictionary<TemplateResource>();
   }
   ...
   public load(args: SLoadArgs) {
      ...
      var template: TemplateResource = templates.get(identity);
      if (template) {
         return template;
      }
      // 创建模板
      template = ClassUtil.create(TemplateResource);
