...
export class Entity extends goog.events.EventTarget {
	...
	defineReadonlyField = function(name, dataAndEvents) {
	...
	static defaultFieldChangedCallback(dataAndEvents, datum, graphics, attributes) {
	...
	_dispatchInvalidateSubgraph = function(deepDataAndEvents) {
	...
	canEdit = function() {
		if(false === this.isFlagOff(EntityFlagEnum.freezed)) {
			return false;
		}
		var i;
		for(i in this.parents) {
	...
	constructor() {
		...
		this.parents = {};
		this.children = {};
		this.boundInternal = new Bound(Infinity, Infinity, 0, 0);
		this.outline = [];
		this._boundDirty = !0;
		this.cache = {};
