...
public class RClass
{
   ...
   public static <V> FClass<V> find(String className){
      ...
      // 创建类对象
      Class<V> nativeClass = null;
      try{
         nativeClass = (Class<V>)Class.forName(className);
         //nativeClass = (Class<V>)_classLoader.loadClass(className);
      }catch(Exception exception){
         throw new FFatalError(exception, "Can't find class. (class_name={1})", className);
      }
