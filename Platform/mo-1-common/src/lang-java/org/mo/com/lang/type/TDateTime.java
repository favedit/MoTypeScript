      ...
      extends MType
{
   ...
   // <T>截取周一单位到周。</T>
   //============================================================
   public void truncateWeekMonday(){
      // 获得日历
      Calendar calendar = Calendar.getInstance();
      calendar.setTimeZone(RDateTime.DEFAULT_TIMEZONE);
      calendar.setTimeInMillis(_value);
      ...
      calendar.set(year, month, monthDay - weekDay, 0, 0, 0);
      _value = calendar.getTimeInMillis();
      // 设置内容
      parse();
   }
   ...
   public String format(String format,
      ...
      if(format == null){
         format = RDateTime.DEFAULT_FORMAT;
      }
      ...
      // 周信息处理
      if(format.contains("wk")){
         TDateTime date = new TDateTime(_value);
         date.truncateWeek();
         format = format.replaceAll("wk", RString.leftPad(Integer.toString(date.day()), 2, split));
         return RDateTime.format(date.get(), format, split);
      }
      if(format.contains("wm")){
         TDateTime date = new TDateTime(_value);
         date.truncateWeekMonday();
         format = format.replaceAll("wm", RString.leftPad(Integer.toString(date.day()), 2, split));
         return RDateTime.format(date.get(), format, split);
      }
      // 格式化字符串
      return RDateTime.format(_value, format, split);
   }
