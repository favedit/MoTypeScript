      ...
      extends MObjects<FRow>
{
   ...
   // @param fields 字段集合
   //============================================================
   public void setFields(SDataField[] fields){
      _fields = fields;
   }
   ...
   // @param name 属性名称
   //============================================================
   public void sortAsDouble(String name){
      sortAsDouble(name, true);
   }
   ...
   public void sortAsDouble(String name,
      ...
      for(int j = 0; j < _count; j++){
         for(int i = 0; i < _count - 1 - j; i++){
            double value1 = RDouble.parse(_items[i].get(name));
            double value2 = RDouble.parse(_items[i + 1].get(name));
            compare = type ? value1 > value2 : value1 < value2;
            if(compare){
               swap(i, i + 1);
            }
   ...
   public String pack(){
      ...
      attributes.set("field_count", (_fields != null) ? _fields.length : 0);
      attributes.set("row_count", _count);
      ...
      // 存储行集合
      if(_fields != null){
         for(SDataField field : _fields){
            String fieldPack = field.pack();
            pack.append(Integer.toString(fieldPack.length()));
            pack.append("_");
            pack.append(fieldPack);
         }
      }
      // 存储行集合
      for(FRow row : this){
         FStrings rowValues = new FStrings();
         for(SDataField field : _fields){
            String value = row.get(field.name, null);
            rowValues.push(RString.nvl(value));
         }
         String rowPack = rowValues.pack();
   ...
   // @param merge 合并标志
   //============================================================
   public boolean unpack(String pack,
                         boolean merge){
      // 是否合并
      if(!merge){
         clear();
      ...
      try{
         ...
         int fieldCount = attributes.getInt("field_count");
         int rowCount = attributes.getInt("row_count");
         position = find + 1 + itemLength;
         // 获得字段集合
         if(fieldCount > 0){
            _fields = new SDataField[fieldCount];
         ...
         for(int n = 0; n < rowCount; n++){
            ...
            FRow row = new FRow();
            FStrings rowValues = new FStrings();
            rowValues.unpack(itemPack);
            for(int i = 0; i < fieldCount; i++){
               SDataField field = _fields[i];
