   ...
   public FDataset fetchDataset(CharSequence sql,
      ...
      try{
         ...
         if(null != resultSet){
            ...
            if(_logger.debugAble()){
               _logger.debugFull(this, "fetchDataset", end - startTime, "Fetch dataset end. (count={0}, total={1}) {2}", dataset.count(), dataset.total(), new FSql(sql.toString()));
            }
         }
      }catch(Exception e){
         exception = e;
         ...
         if(exception != null){
            throw new FFatalError(exception, "Fetch dataset failure. (sql={1}, page_size={2}, page={3})", sql, pageSize, page);
         }
      }
      return dataset;
   }
