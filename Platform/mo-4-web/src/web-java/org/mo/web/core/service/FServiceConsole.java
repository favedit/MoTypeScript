...
import org.mo.web.protocol.context.IWebJsonInput;
import org.mo.web.protocol.context.IWebJsonOutput;
import org.mo.web.protocol.context.IWebOutput;
import org.mo.web.protocol.context.IWebXmlInput;
import org.mo.web.protocol.context.IWebXmlOutput;
         ...
         IServiceConsole
{
   ...
   protected void buildMessages(IWebContext context,
      ...
      if(!messages.isEmpty()){
         if(output instanceof FWebXmlOutput){
            FXmlNode xoutput = ((FWebXmlOutput)output).config();
            // 清空输出内容
            xoutput.nodes().clear();
            // 创建消息集合节点
   ...
   public EResult checkLogin(IWebContext context,
      ...
      if(!session.user().isLogin()){
         ...
         fatalNode.set("redirect", _messageConsole.pageLogin());
         return null;
      }
      return EResult.Success;
   }
   ...
   public Object execute(String name,
      ...
      try{
         for(int n = 0; n < paramCount; n++){
            ...
            }else if((type == IWebInput.class) || (type == IWebJsonInput.class) || (type == IWebXmlInput.class)){
               // 参数对象为网络输入对象
               value = input;
            }else if((type == IWebOutput.class) || (type == IWebJsonOutput.class) || (type == IWebXmlOutput.class)){
               // 参数对象为网络输出对象
               value = output;
